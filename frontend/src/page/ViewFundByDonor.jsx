...
<tbody>
  {funds.map((fund) => (
    <tr key={fund._id}>
      ...
      <td>{new Date(fund.startDate).toLocaleDateString()}</td>
      <td>{new Date(fund.endDate).toLocaleDateString()}</td>
      <td>{fund.amount}</td>
      <td>{fund.status}</td>
      <td>{fund.collectedAmount}</td>
      <td>
        {fund.donors?.length > 0 ? (
          <ul className="list-unstyled text-start">
            {fund.donors.map((donor, index) => (
              <li key={index}>
                <strong>User ID:</strong> {donor.userId?._id || donor.userId} <br />
                <strong>Donation:</strong> ₹{donor.donationAmount}
              </li>
            ))}
          </ul>
        ) : (
...
export default viewfundbydonor
